...
"""
    This Version: $Id: 3ds2egg.py,v 1.19 2008/07/13 18:27:24 andyp Exp $
...
class TDSChunk:
    ...
    # CHUNK_ATTRIB[0x0012] = { "container": False, "name": "gcolor24" }
    # CHUNK_ATTRIB[0x0013] = { "container": False, "name": "gcolorf" }
    ...
    CHUNK_ATTRIB[0x4600] = { "container": False, "name": "light" }
    ...
    CHUNK_ATTRIB[0xa042] = { "container":  True, "name": "shininess3" }
    ...
    CHUNK_ATTRIB[0xa204] = { "container":  True, "name": "specularmap" }
    CHUNK_ATTRIB[0xa210] = { "container":  True, "name": "opacitymap" }
    CHUNK_ATTRIB[0xa220] = { "container":  True, "name": "reflectionmap" }
    CHUNK_ATTRIB[0xa230] = { "container":  True, "name": "bumpmap" }
    CHUNK_ATTRIB[0xa252] = { "container": False, "name": "bumpamount" }
    CHUNK_ATTRIB[0xa300] = { "container": False, "name": "mapname" }
    CHUNK_ATTRIB[0xa33a] = { "container":  True, "name": "texturemap2" }
    CHUNK_ATTRIB[0xa33c] = { "container":  True, "name": "shininessmap" }
    CHUNK_ATTRIB[0xa33d] = { "container":  True, "name": "selfillummap" }
    CHUNK_ATTRIB[0xa351] = { "container": False, "name": "maptiling" }
    CHUNK_ATTRIB[0xa353] = { "container": False, "name": "mapfilter" }
    # CHUNK_ATTRIB[0xa354] = { "container": False, "name": "vscale" }
    # CHUNK_ATTRIB[0xa356] = { "container": False, "name": "uscale" }
    # CHUNK_ATTRIB[0xa358] = { "container": False, "name": "uoffset" }
    # CHUNK_ATTRIB[0xa35a] = { "container": False, "name": "voffset" }
    ...
    def getEggMaterial(self):
        ...
        if name is not None:
            ...
            if Ks is not None:
                ...
                if Ns is not None:
                    Ns = float(Ns) / 100.0
                    m.setSpec(Vec4(Ns*Ks[0], Ns*Ks[1], Ns*Ks[2], 1.0))
            Ns2 = self.get("shininess2")
            if Ns2 is not None:
                m.setShininess(float(Ns2)/100.0 * 128.0)
            self.eggmaterial = m
        return self.eggmaterial
...
class ChunkAmbient(TDSChunk):
    def __init__(self, parent=None):
        TDSChunk.__init__(self, parent)
        # per-pixel lighting seems to want non-zero ambient.  bug?
        # NB: these values are in the range 0 to 255 here!
        self.put("color", (1, 1, 1))
    ...
    def eggifymaterials(self, rootchunk, egg):
        ...
        if (c[0] == 0.0) and (c[1] == 0.0) and (c[2] == 0.0):
            c = [0.01, 0.01, 0.01]
        self.parent.put("ambient", c)
        return self

class ChunkDiffuse(TDSChunk):
    def __init__(self, parent=None):
        TDSChunk.__init__(self, parent)
        self.put("color", (0.99, 0.99, 0.99))
...
class ChunkShininess(TDSChunk):
    def __init__(self, parent=None):
        TDSChunk.__init__(self, parent)
        self.put("percentage", 0)

    def eggifymaterials(self, rootchunk, egg):
        TDSChunk.eggifymaterials(self, rootchunk, egg)
        self.parent.put("shininess", self.get("percentage"))
        return self

class ChunkShininess2(TDSChunk):
    def __init__(self, parent=None):
        TDSChunk.__init__(self, parent)
        self.put("percentage", 0)
    ...
    def eggifymaterials(self, rootchunk, egg):
        TDSChunk.eggifymaterials(self, rootchunk, egg)
        self.parent.put("shininess2", self.get("percentage"))
        return self

class ChunkShininess3(TDSChunk):
    def __init__(self, parent=None):
        TDSChunk.__init__(self, parent)
        self.put("percentange", 0)

    def eggifymaterials(self, rootchunk, egg):
        TDSChunk.eggifymaterials(self, rootchunk, egg)
        self.parent.put("shininess3", self.get("percentage"))
        return self
...
class ChunkBumpmap(TDSChunk):
    ...
    def eggifymaterials(self, rootchunk, egg):
        TDSChunk.eggifymaterials(self, rootchunk, egg)
        self.parent.put("bumpmap", self.get("mapname"))
        return self
...
class ChunkBumpAmount(TDSChunk):
    def __init__(self, parent=None):
        TDSChunk.__init__(self, parent)
        self.put("bumpamount", 0)
    ...
    def eggifymaterials(self, rootchunk, egg):
        ...
        data = rootchunk.data
        base = self.base + 6
        value = struct.unpack("<H", data[base:base+2])[0]
        self.put("bumpamount", value)
        self.parent.put("bumpamount", value)
        return self

class ChunkMapTiling(TDSChunk):
    def __init__(self, parent=None):
        TDSChunk.__init__(self, parent)
        self.put("maptiling", 0)
    ...
    def eggifymaterials(self, rootchunk, egg):
        ...
        base = self.base + 6
        value = struct.unpack("<H", data[base:base+2])[0]
        self.put("maptiling", value)
        self.parent.put("maptiling", value)
        return self

class ChunkMapFilter(TDSChunk):
    def __init__(self, parent=None):
        TDSChunk.__init__(self, parent)
        self.put("mapfiltering", 0.0)

    def eggifymaterials(self, rootchunk, egg):
        # 1 float after the header...
        data = rootchunk.data
        base = self.base + 6
        value = struct.unpack("<f", data[base:base+4])[0]
        self.put("mapfiltering", value)
        self.parent.put("mapfiltering", value)
        return self

class ChunkLight(TDSChunk):
    def __init__(self, parent=None):
        TDSChunk.__init__(self, parent)
        self.put("pos", (0.0, 0.0, 0.0))

    def eggifygeometry(self, rootchunk, egg):
        data = rootchunk.data
        base = self.base + 6
        geom = struct.unpack("<fff", data[base:base+12])
        self.put("pos", geom)
        return self
...
def initChunkMakers():
    chunkmakers = [
        ...
        ( 0xa041, ChunkShininess2 ),
        ( 0xa042, ChunkShininess3 ),
        ( 0xa050, ChunkTransparency ),
        ( 0xa081, ChunkTwoSided ),
        ( 0xa200, ChunkTexturemap ),
        ( 0xa230, ChunkBumpmap ),
        ( 0xa252, ChunkBumpAmount ),
        ( 0xa300, ChunkMapname ),
        ( 0xa351, ChunkMapTiling ),
        ( 0xa353, ChunkMapFilter ),
...
def main(argv=None):
    if argv is None:
        argv = sys.argv
    try:
        opts, args = getopt.getopt(argv[1:], "hn:bsp", ["help", "normals", "binormals", "show", "polynormals"])
    except getopt.error, msg:
        print msg
        print __doc__
        return 2
    show = False
    do_polynormals = False
    for o, a in opts:
        if o in ("-h", "--help"):
            print __doc__
            return 0
