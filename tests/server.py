...
#loadPrcFileData("", "notify-level-windisplay spam")
...
class PSGServer():
	...
	def __handleCommand(self):
		...
		if (len(self._commandTok) > 0):
			if self._commandTok[0] == 'exit':
				self.__quit()
			if self._commandTok[0] == 'lsconn':
				self.__lsConnections()
			if self._commandTok[0] == 'lsusers':
				self.__lsUsers()
			if self._commandTok[0] == 'lsgames':
				self.__lsGames()
			if self._commandTok[0] == 'kill':
				self.__kill()
			#else: # Unknown command
			#	self.__printLine('Unknown command, I understand these:')
			#	self.__printLine('lsconn      lsuser     lsgames')
	...
	def __kill(self):
		if (len(self._commandTok) == 2):
			if (self._commandTok[1][0:2] == '-u'):
				username = int(self._commandTok[1].split('=')[1])
				self.__printLine('Killing user %d'%(username))
			if (self._commandTok[1][0:2] == '-g'):
				gameid = int(self._commandTok[1].split('=')[1])
				self.__printLine('Killing game %d'%(gameid))
				
		else:
			self.__printLine('USAGE: kill [OPTION]')
			self.__printLine('  -u=username      Bump user from all games and disconnect')
			self.__printLine('  -g=gameid        End the game leaving users connected')
